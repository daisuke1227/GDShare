...
static Task<Result<std::filesystem::path>> promptExportLevel(L* level) {
    ...
    std::string base = std::is_same_v<L, GJLevelList>
...
struct $modify(ExportMyLevelLayer, EditLevelLayer) {
    ...
    bool init(GJGameLevel* level) {
        ...
        if (auto m = getChildByID("level-actions-menu")) {
...
struct $modify(ExportOnlineLevelLayer, LevelInfoLayer) {
    struct Fields { EventListener<Task<Result<std::filesystem::path>>> pickListener; };
    $override
    bool init(GJGameLevel* level,bool c){ if(!LevelInfoLayer::init(level,c))return false;
...
struct $modify(ImportLayer, LevelBrowserLayer) {
    ...
    static void importFiles(const std::vector<std::filesystem::path>& p){
        for(auto& f:p){switch(getGmdFileKind(f)){
            case GmdFileKind::List:{auto r=importGmdAsList(f);if(r)LocalLevelManager::get()->m_localLists->insertObject(*r,0);
                else{FLAlertLayer::create("Error Importing",r.unwrapErr(),"OK")->show();return;}}break;
            case GmdFileKind::Level:{auto r=importGmdAsLevel(f);if(r)LocalLevelManager::get()->m_localLevels->insertObject(*r,0);
                else{FLAlertLayer::create("Error Importing",r.unwrapErr(),"OK")->show();return;}}break;
            case GmdFileKind::None:
                FLAlertLayer::create("Error Importing",fmt::format("Selected '<cp>{}</c>' not GMD!",f.string()),"OK")->show();return;
        }}auto sc=CCScene::create();auto ly=LevelBrowserLayer::create(GJSearchObject::create(SearchType::MyLevels));sc->addChild(ly);
        CCDirector::sharedDirector()->replaceScene(CCTransitionFade::create(.5f,sc));
    }
    void onImport(CCObject*){
        m_fields->pickListener.bind([](auto*e){if(auto r=e->getValue();r&&r->isOk())importFiles(r->unwrap());else if(r)FLAlertLayer::create("Error Importing",r->unwrapErr(),"OK")->show();});
        m_fields->pickListener.setFilter(file::pickMany(IMPORT_PICK_OPTIONS));
    }
    $override
    bool init(GJSearchObject*s){if(!LevelBrowserLayer::init(s))return false;
        if(s->m_searchType==SearchType::MyLevels||s->m_searchType==SearchType::MyLists){auto m=getChildByID("new-level-menu");auto ib=CCMenuItemSpriteExtra::create(
...
struct $modify(ExportListLayer, LevelListLayer) {
    struct Fields { EventListener<Task<Result<std::filesystem::path>>> pickListener; };
    $override
    bool init(GJLevelList* l){if(!LevelListLayer::init(l))return false;
        if(auto m=getChildByID("left-side-menu")){auto b=CCMenuItemSpriteExtra::create(
                CircleButtonSprite::createWithSpriteFrameName("file.png"_spr,.8f,CircleBaseColor::Green,CircleBaseSize::Medium),
                this,menu_selector(ExportListLayer::onExport)
            );b->setID("export-button"_spr);m->addChild(b);m->updateLayout();}
        return true;
    }
    void onExport(CCObject*){
